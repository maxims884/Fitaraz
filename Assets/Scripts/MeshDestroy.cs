...
public class MeshDestroy : MonoBehaviour
{
    ...
    private int CutCascades = 5;
    private float ExplodeForce = 5;
    public void DestroyMesh(GameObject obj)
    {
        destroyedObject = obj;

        var originalMesh = destroyedObject.GetComponent<MeshFilter>().mesh;
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            parts[i].MakeGameobject(destroyedObject,CutCascades,ExplodeForce);
            //parts[i].GameObject.GetComponent<Rigidbody>().AddForceAtPosition(parts[i].Bounds.center * ExplodeForce, destroyedObject.transform.position);
            parts[i].GameObject.GetComponent<Rigidbody>().drag = 10;
            parts[i].GameObject.GetComponent<Rigidbody>().useGravity = false;
        }

        UnityEngine.Object.Destroy(destroyedObject);
    }
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(GameObject original, int CutCascades, float ExplodeForce)
        {
            ...
            meshDestroy.ExplodeForce = ExplodeForce;
